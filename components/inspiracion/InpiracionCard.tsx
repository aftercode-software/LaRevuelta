...
import { InspirationCard } from "@/interfaces/inspiracion";

export default function InspiracionCard({ inspo }: { inspo: InspirationCard }) {
  ...
  return (
    <Link
      ...
      className="relative flex flex-col justify-end items-center  rounded-xl overflow-hidden group text-center w-full aspect-square  sm:max-w-full mx-auto cursor-pointer "
    ...
    >
      ...
      <div className="relative z-30 flex flex-col w-full text-left p-5 sm:px-8">
        ...
        <p className="font-geist text-lg sm:text-lg mt-1">{inspo.desc}</p>
      </div>
    </Link>
  );
}
